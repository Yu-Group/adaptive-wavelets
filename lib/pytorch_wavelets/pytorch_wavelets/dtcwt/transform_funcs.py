...
class FWD_J1(Function):
    ...
    @staticmethod
    def forward(x, h0, h1, skip_hps, o_dim, ri_dim, mode):
        mode = int_to_mode(mode)
        dims = get_dimensions5(o_dim, ri_dim)
        o_dim, ri_dim = dims[0], dims[1]
...
class FWD_J2PLUS(Function):
    ...
    @staticmethod
    def forward(x, h0a, h1a, h0b, h1b, skip_hps, o_dim, ri_dim, mode):
        mode = 'symmetric'
        dims = get_dimensions5(o_dim, ri_dim)
        o_dim, ri_dim = dims[0], dims[1]
...
class INV_J1(Function):
    ...
    @staticmethod
    def forward(lows, highs, g0, g1, o_dim, ri_dim, mode):
        mode = int_to_mode(mode)
        dims = get_dimensions5(o_dim, ri_dim)
        o_dim, ri_dim, h_dim, w_dim = dims
...
class INV_J2PLUS(Function):
    ...
    @staticmethod
    def forward(lows, highs, g0a, g1a, g0b, g1b, o_dim, ri_dim, mode):
        mode = 'symmetric'
        dims = get_dimensions5(o_dim, ri_dim)
        o_dim, ri_dim, h_dim, w_dim = dims
        if highs is not None and highs.shape != torch.Size([]):
